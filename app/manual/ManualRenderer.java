...
import com.example.pista.aut.MainActivity;
    ...
    private void updateCubePosition() {
        if (currentDirection != MovementDirection.NONE) {
            ...
            switch (currentDirection) {
                case FORWARD:
                    newX += moveSpeed * Math.sin(Math.toRadians(moveAngle));
                    newZ += moveSpeed * Math.cos(Math.toRadians(moveAngle));
                    break;
                case BACKWARD:
                    newX -= moveSpeed * Math.sin(Math.toRadians(moveAngle));
                    newZ -= moveSpeed * Math.cos(Math.toRadians(moveAngle));
