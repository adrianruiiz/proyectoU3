...
public class GameRenderer implements GLSurfaceView.Renderer {
    ...
    private Engine engine;             // Motor de Filament
    private Renderer renderer;         // Renderizador de Filament
    private Scene scene;               // Escena de Filament
    private View view;                 // Vista de Filament
    private SwapChain swapChain;       // SwapChain para el renderizado
    ...
    public GameRenderer(Context context, GLSurfaceView glSurfaceView, ArrayList<Float> pathPoints) {
        this.context = context;
        this.glSurfaceView = glSurfaceView; // Guardar referencia al GLSurfaceView
        this.customTrackPoints = pathPoints;
        this.player = new Player();

        this.trackPoints = pathPoints;
        Matrix.setIdentityM(modelMatrix, 0);
        car = null; // Se inicializa en `onSurfaceCreated`.
    }
    ...
    public void placeCarAt(float x, float y, float z) {
        if (isPointOnTrack(x, z)) {
            ...
            Log.d("GameRenderer", "Carro colocado en: " + x + ", " + y + ", " + z);
        } else {
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        try {
            ...
            car = new CarObject(context, engine, scene);
    ...
    public void handleTouch(float screenX, float screenY) {
        ...
        Log.d(TAG, "Carro colocado en: " + worldCoords[0] + ", " + worldCoords[2]);
    }

    @Override
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        if (view != null) {
            // Crear un SwapChain para la superficie de renderizado
            swapChain = engine.createSwapChain(glSurfaceView.getHolder().getSurface());

            // Crear el objeto Viewport y configurarlo
    ...
    public void onDrawFrame(GL10 gl) {
        try {
            ...
            // Renderizar la escena con Filament
            ...
            // Movimiento automático del coche
    ...
    private void updatePlayerMovement() {
        ...
        if (deltaX != 0 || deltaZ != 0) {
            ...
            if (isPointOnTrack(newX, newZ)) {
                ...
                Log.d(TAG, "Carro movido a: (" + newX + ", " + newZ + ")");
            } else {
